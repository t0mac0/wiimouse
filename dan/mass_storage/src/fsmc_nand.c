...
void FSMC_NAND_ReadID(NAND_IDTypeDef* NAND_ID)
{
  ...
  *(__IO uint8 *)(Bank_NAND_ADDR | CMD_AREA) = 0x90;
  *(__IO uint8 *)(Bank_NAND_ADDR | ADDR_AREA) = 0x00;

   /* Sequence to read ID from NAND flash */	
   data = *(__IO uint32 *)(Bank_NAND_ADDR | DATA_AREA);
...
uint32 FSMC_NAND_WriteSmallPage(uint8 *pBuffer, NAND_ADDRESS Address, uint32 NumPageToWrite)
{
  ...
  while((NumPageToWrite != 0x00) && (addressstatus == NAND_VALID_ADDRESS) && (status == NAND_READY))
  {
    ...
    *(__IO uint8 *)(Bank_NAND_ADDR | CMD_AREA) = NAND_CMD_AREA_A;
    *(__IO uint8 *)(Bank_NAND_ADDR | CMD_AREA) = NAND_CMD_WRITE0;

    *(__IO uint8 *)(Bank_NAND_ADDR | ADDR_AREA) = 0x00;  
    *(__IO uint8 *)(Bank_NAND_ADDR | ADDR_AREA) = ADDR_1st_CYCLE(ROW_ADDRESS);  
    *(__IO uint8 *)(Bank_NAND_ADDR | ADDR_AREA) = ADDR_2nd_CYCLE(ROW_ADDRESS);  
    *(__IO uint8 *)(Bank_NAND_ADDR | ADDR_AREA) = ADDR_3rd_CYCLE(ROW_ADDRESS);  
    ...
    *(__IO uint8 *)(Bank_NAND_ADDR | CMD_AREA) = NAND_CMD_WRITE_TRUE1;
...
uint32 FSMC_NAND_ReadSmallPage(uint8 *pBuffer, NAND_ADDRESS Address, uint32 NumPageToRead)
{
  ...
  {	   
    ...
    *(__IO uint8 *)(Bank_NAND_ADDR | CMD_AREA) = NAND_CMD_AREA_TRUE1; 
...
{
  uint32 index = 0x00, numsparesreawritten = 0x00, addressstatus = NAND_VALID_ADDRESS;
  uint32 status = NAND_READY, size = 0x00; 

  while((NumSpareAreaTowrite != 0x00) && (addressstatus == NAND_VALID_ADDRESS) && (status == NAND_READY))
  {
    /* Page write Spare area command and address */
    *(__IO uint8 *)(Bank_NAND_ADDR | CMD_AREA) = NAND_CMD_AREA_C;
    *(__IO uint8 *)(Bank_NAND_ADDR | CMD_AREA) = NAND_CMD_WRITE0;
...
{
  uint32 numsparearearead = 0x00, index = 0x00, addressstatus = NAND_VALID_ADDRESS;
  uint32 status = NAND_READY, size = 0x00;
...
{
  uint32 status = NAND_VALID_ADDRESS;
