...
/* Private variables ---------------------------------------------------------*/
uint8 Bot_State;
uint8 Bulk_Data_Buff[BULK_MAX_PACKET_SIZE];  /* data buffer*/
uint16 Data_Len;
Bulk_Only_CBW CBW;
Bulk_Only_CSW CSW;
uint32 SCSI_LBA , SCSI_BlkLen;
extern uint32 Max_Lun;
/* Extern variables ----------------------------------------------------------*/
...
{
	printf("mass storage in\n");
  switch (Bot_State)
  {
    case BOT_CSW_Send:
    case BOT_ERROR:
    	if(Bot_State == BOT_ERROR) printf("in error\n");
      Bot_State = BOT_IDLE;
...
void Mass_Storage_Out (void)
{
  ...
  switch (Bot_State)
  {
    ...
    case BOT_DATA_OUT:
      ...
      printf("CMD == SCSI_WRITE10: CSW_PHASE_ERROR\n");
...
void CBW_Decode(void)
{
  ...
  {
    *((uint8 *)&CBW + Counter) = Bulk_Data_Buff[Counter];
  }
  CSW.dTag = CBW.dTag;
...
* Input          : uint8* Data_Address : point to the data to transfer.
...
* Input          : uint8 CSW_Status this filed can be CSW_CMD_PASSED,CSW_CMD_FAILED,
*                  or CSW_PHASE_ERROR.
* Output         : None.
* Return         : None.
*******************************************************************************/
void Set_CSW (uint8 CSW_Status, uint8 Send_Permission)
{
  ...
  USB_SIL_Write(EP1_IN, ((uint8 *)& CSW), CSW_DATA_LENGTH);

  Bot_State = BOT_ERROR;
  if (Send_Permission)
  {
	  printf("Sending CWS status\n");
