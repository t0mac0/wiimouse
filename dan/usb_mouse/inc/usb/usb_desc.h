///******************** (C) COPYRIGHT 2010 STMicroelectronics ********************
//* File Name          : usb_desc.h
//* Author             : MCD Application Team
//* Version            : V3.2.1
//* Date               : 07/05/2010
//* Description        : Descriptor Header for Custom HID Demo
//********************************************************************************
//* THE PRESENT FIRMWARE WHICH IS FOR GUIDANCE ONLY AIMS AT PROVIDING CUSTOMERS
//* WITH CODING INFORMATION REGARDING THEIR PRODUCTS IN ORDER FOR THEM TO SAVE TIME.
//* AS A RESULT, STMICROELECTRONICS SHALL NOT BE HELD LIABLE FOR ANY DIRECT,
//* INDIRECT OR CONSEQUENTIAL DAMAGES WITH RESPECT TO ANY CLAIMS ARISING FROM THE
//* CONTENT OF SUCH FIRMWARE AND/OR THE USE MADE BY CUSTOMERS OF THE CODING
//* INFORMATION CONTAINED HEREIN IN CONNECTION WITH THEIR PRODUCTS.
//*******************************************************************************/
//
///* Define to prevent recursive inclusion -------------------------------------*/
//#ifndef __USB_DESC_H
//#define __USB_DESC_H
//
//#include "usb.h"
//#include "usb_hid.h"
//#include "usb_hid_mouse.h"
//
///* Includes ------------------------------------------------------------------*/
///* Exported types ------------------------------------------------------------*/
///* Exported constants --------------------------------------------------------*/
///* Exported macro ------------------------------------------------------------*/
///* Exported define -----------------------------------------------------------*/
//#define CUSTOMHID_OFF_HID_DESC                  0x12
//
//#define CUSTOMHID_SIZ_CONFIG_DESC               41
//
//#define CUSTOMHID_SIZ_STRING_LANGID             4
//#define CUSTOMHID_SIZ_STRING_VENDOR             38
//#define CUSTOMHID_SIZ_STRING_PRODUCT            32
//#define CUSTOMHID_SIZ_STRING_SERIAL             26
//
//
///* Exported functions ------------------------------------------------------- */
//extern const uint8 CustomHID_DeviceDescriptor[USB_DEVICE_DESCRIPTOR_SIZE];
//extern const uint8 CustomHID_ConfigDescriptor[CUSTOMHID_SIZ_CONFIG_DESC];
//extern const uint8 CustomHID_ReportDescriptor[HID_MOUSE_REPORT_DESCRIPTOR_SIZE];
//extern const uint8 CustomHID_StringLangID[CUSTOMHID_SIZ_STRING_LANGID];
//extern const uint8 CustomHID_StringVendor[CUSTOMHID_SIZ_STRING_VENDOR];
//extern const uint8 CustomHID_StringProduct[CUSTOMHID_SIZ_STRING_PRODUCT];
//extern uint8 CustomHID_StringSerial[CUSTOMHID_SIZ_STRING_SERIAL];
//
//#endif /* __USB_DESC_H */
//
///******************* (C) COPYRIGHT 2010 STMicroelectronics *****END OF FILE****/
/******************** (C) COPYRIGHT 2010 STMicroelectronics ********************
* File Name          : usb_desc.h
* Author             : MCD Application Team
* Version            : V3.2.1
* Date               : 07/05/2010
* Description        : Descriptor Header for Custom HID Demo
********************************************************************************
* THE PRESENT FIRMWARE WHICH IS FOR GUIDANCE ONLY AIMS AT PROVIDING CUSTOMERS
* WITH CODING INFORMATION REGARDING THEIR PRODUCTS IN ORDER FOR THEM TO SAVE TIME.
* AS A RESULT, STMICROELECTRONICS SHALL NOT BE HELD LIABLE FOR ANY DIRECT,
* INDIRECT OR CONSEQUENTIAL DAMAGES WITH RESPECT TO ANY CLAIMS ARISING FROM THE
* CONTENT OF SUCH FIRMWARE AND/OR THE USE MADE BY CUSTOMERS OF THE CODING
* INFORMATION CONTAINED HEREIN IN CONNECTION WITH THEIR PRODUCTS.
*******************************************************************************/

/* Define to prevent recursive inclusion -------------------------------------*/
#ifndef __USB_DESC_H
#define __USB_DESC_H

#include "stm32f10x_type.h"
#include "usb.h"
#include "usb_hid.h"
#include "usb_hid_mouse.h"
#include "usb_conf.h"

/* Includes ------------------------------------------------------------------*/
/* Exported types ------------------------------------------------------------*/
/* Exported constants --------------------------------------------------------*/
/* Exported macro ------------------------------------------------------------*/
/* Exported define -----------------------------------------------------------*/

//#define CUSTOMHID_SIZ_HID_DESC                  0x09
#define CUSTOMHID_OFF_HID_DESC                  USB_CONFIGURATION_DESCRIPTOR_SIZE +USB_INTERFACE_DESCRIPTOR_SIZE

//#define CUSTOMHID_SIZ_DEVICE_DESC               18
//#define CUSTOMHID_SIZ_CONFIG_DESC               34//41
//#define CUSTOMHID_SIZ_REPORT_DESC               50//163
#define CUSTOMHID_SIZ_STRING_LANGID             4
#define CUSTOMHID_SIZ_STRING_VENDOR             38
#define CUSTOMHID_SIZ_STRING_PRODUCT            32
#define CUSTOMHID_SIZ_STRING_SERIAL             26

//#define STANDARD_ENDPOINT_DESC_SIZE             0x09

/* Exported functions ------------------------------------------------------- */
extern const uint8 CustomHID_DeviceDescriptor[USB_DEVICE_DESCRIPTOR_SIZE];
extern const uint8 CustomHID_ConfigDescriptor[WII_MOUSE_CONFIGURATION_DESCRIPTOR_SIZE];
extern const uint8 CustomHID_ReportDescriptor[HID_MOUSE_REPORT_DESCRIPTOR_SIZE];
extern const uint8 CustomHID_StringLangID[CUSTOMHID_SIZ_STRING_LANGID];
extern const uint8 CustomHID_StringVendor[CUSTOMHID_SIZ_STRING_VENDOR];
extern const uint8 CustomHID_StringProduct[CUSTOMHID_SIZ_STRING_PRODUCT];
extern uint8 CustomHID_StringSerial[CUSTOMHID_SIZ_STRING_SERIAL];

#endif /* __USB_DESC_H */

/******************* (C) COPYRIGHT 2010 STMicroelectronics *****END OF FILE****/
