...
#define EVOE_BitNumber              ((uint8)0x07)
#define EVCR_EVOE_BB                (PERIPH_BB_BASE + (EVCR_OFFSET * 32) + (EVOE_BitNumber * 4))

#define EVCR_PORTPINCONFIG_MASK     ((uint16)0xFF80)
#define LSB_MASK                    ((uint16)0xFFFF)
#define DBGAFR_POSITION_MASK        ((uint32)0x000F0000)
#define DBGAFR_SWJCFG_MASK          ((uint32)0xF8FFFFFF)
...
uint8 GPIO_ReadInputDataBit(GPIO_TypeDef* GPIOx, uint16 GPIO_Pin)
{
  ...
  if ((GPIOx->IDR & GPIO_Pin) != (uint32)Bit_RESET)
  {
...
uint8 GPIO_ReadOutputDataBit(GPIO_TypeDef* GPIOx, uint16 GPIO_Pin)
{
  ...
  if ((GPIOx->ODR & GPIO_Pin) != (uint32)Bit_RESET)
  {
    bitstatus = (uint8)Bit_SET;
  }
  else
...
void GPIO_EventOutputConfig(uint8 GPIO_PortSource, uint8 GPIO_PinSource)
{
  ...
  tmpreg |= (uint32)GPIO_PortSource << 0x04;
...
void GPIO_EventOutputCmd(FunctionalState NewState)
{
  ...
  *(vuint32 *) EVCR_EVOE_BB = (uint32)NewState;
}
...
{
  uint32 tmp = 0x00, tmp1 = 0x00, tmpreg = 0x00, tmpmask = 0x00;
...
void GPIO_EXTILineConfig(uint8 GPIO_PortSource, uint8 GPIO_PinSource)
{
  ...
  AFIO->EXTICR[GPIO_PinSource >> 0x02] |= (((uint32)GPIO_PortSource) << (0x04 * (GPIO_PinSource & (uint8)0x03)));
}
