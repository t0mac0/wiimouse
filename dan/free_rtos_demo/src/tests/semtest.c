...
short sError = pdFALSE, sCheckVariableToUse;
	...
	for( ;; )
	{
		...
		if( xSemaphoreTake( pxParameters->xSemaphore, pxParameters->xBlockTime ) == pdPASS )
		{
			...
			{
				sError = pdTRUE;
				printf("prvSemaphoreTest: *pulSharedVariable != ulExpectedValue\n");
			}
			...
			for( ulCounter = ( unsigned long ) 0; ulCounter <= ulExpectedValue; ulCounter++ )
			{
				*pulSharedVariable = ulCounter;
				...
				{
					sError = pdTRUE;
					printf("prvSemaphoreTest: *pulSharedVariable != ulCounter\n");
				}
			}
			...
			{
				sError = pdTRUE;
				printf("prvSemaphoreTest: xSemaphoreGive\n");
			}
...
portBASE_TYPE xTask, xReturn = pdTRUE;
	...
	for( xTask = 0; xTask < semtstNUM_TASKS; xTask++ )
	{
		...
		{
			printf("sLastCheckVariables[ xTask ] (%d) == sCheckVariables[ xTask ] (%d) \n", sLastCheckVariables[ xTask ],sCheckVariables[ xTask ] );
			xReturn = pdFALSE;
		}
