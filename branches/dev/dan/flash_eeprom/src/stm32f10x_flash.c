...
#define OB_USER_BFB2             ((uint16)0x0008)
...
FLASH_Status FLASH_UserOptionByteConfig(uint16 OB_IWDG, uint16 OB_STOP, uint16 OB_STDBY)
{
  ...
  {  
    ...
    OB->USER = OB_IWDG | (uint16)(OB_STOP | (uint16)(OB_STDBY | ((uint16)0xF8)));
...
{ 
  ...
  {  
    ...
    {
      OB->USER &= (uint16)(~(uint16)(OB_USER_BFB2));
    }
    /* Wait for last operation to be completed */
    status = FLASH_WaitForLastOperation(ProgramTimeout);
    if(status != FLASH_TIMEOUT)
...
void FLASH_ITConfig(uint32 FLASH_IT, FunctionalState NewState)
{
  ...
  if((FLASH_IT & 0x80000000) != 0x0)
  {
    ...
    else
    {
      ...
      FLASH->CR2 &= ~(uint32)(FLASH_IT & 0x7FFFFFFF);
    }
  }
  else
  {
...
FlagStatus FLASH_GetFlagStatus(uint32 FLASH_FLAG)
{
  ...
  else
  {
    ...
    {
      if((FLASH->SR2 & FLASH_FLAG) != (uint32)RESET)
      {
        bitstatus = SET;
      }
      else
