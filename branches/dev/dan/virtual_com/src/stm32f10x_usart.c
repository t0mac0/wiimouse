...
/* Private define ------------------------------------------------------------*/
/* USART RUN Mask */
#define CR1_RUN_Set               ((uint16)0x2000)  /* USART Enable Mask */
#define CR1_RUN_Reset             ((uint16)0xDFFF)  /* USART Disable Mask */

#define CR2_Address_Mask          ((uint16)0xFFF0)  /* USART address Mask */

/* USART RWU Mask */
#define CR1_RWU_Set               ((uint16)0x0002)  /* USART mute mode Enable Mask */
#define CR1_RWU_Reset             ((uint16)0xFFFD)  /* USART mute mode Enable Mask */

#define USART_IT_Mask             ((uint16)0x001F)  /* USART Interrupt Mask */

/* USART LIN Mask */
#define CR2_LINE_Set              ((uint16)0x4000)  /* USART LIN Enable Mask */
#define CR2_LINE_Reset            ((uint16)0xBFFF)  /* USART LIN Disable Mask */

#define CR1_SBK_Set               ((uint16)0x0001)  /* USART Break Character send Mask */

/* USART SC Mask */
#define CR3_SCEN_Set              ((uint16)0x0020)  /* USART SC Enable Mask */
#define CR3_SCEN_Reset            ((uint16)0xFFDF)  /* USART SC Disable Mask */

/* USART SC NACK Mask */
#define CR3_NACK_Set              ((uint16)0x0010)  /* USART SC NACK Enable Mask */
#define CR3_NACK_Reset            ((uint16)0xFFEF)  /* USART SC NACK Disable Mask */

/* USART Half-Duplex Mask */
#define CR3_HDSEL_Set             ((uint16)0x0008)  /* USART Half-Duplex Enable Mask */
#define CR3_HDSEL_Reset           ((uint16)0xFFF7)  /* USART Half-Duplex Disable Mask */

/* USART IrDA Mask */
#define CR3_IRLP_Mask             ((uint16)0xFFFB)  /* USART IrDA LowPower mode Mask */

/* USART LIN Break detection */
#define CR3_LBDL_Mask             ((uint16)0xFFDF)  /* USART LIN Break detection Mask */

/* USART WakeUp Method  */
#define CR3_WAKE_Mask             ((uint16)0xF7FF)  /* USART WakeUp Method Mask */

/* USART IrDA Mask */
#define CR3_IREN_Set              ((uint16)0x0002)  /* USART IrDA Enable Mask */
#define CR3_IREN_Reset            ((uint16)0xFFFD)  /* USART IrDA Disable Mask */

#define GTPR_LSB_Mask             ((uint16)0x00FF)  /* Guard Time Register LSB Mask */
#define GTPR_MSB_Mask             ((uint16)0xFF00)  /* Guard Time Register MSB Mask */

#define CR1_CLEAR_Mask            ((uint16)0xE9F3)  /* USART CR1 Mask */
#define CR2_CLEAR_Mask            ((uint16)0xC0FF)  /* USART CR2 Mask */
...
{
  uint32 tmpreg = 0x00, apbclock = 0x00;
...
/*---------------------------- USART CR2 Configuration -----------------------*/
  ...
  tmpreg |= (uint32)USART_InitStruct->USART_StopBits | USART_InitStruct->USART_Clock |
...
/*---------------------------- USART CR1 Configuration -----------------------*/
  ...
  tmpreg |= (uint32)USART_InitStruct->USART_WordLength | USART_InitStruct->USART_Parity |
...
{
  uint32 usartreg = 0x00, itpos = 0x00, itmask = 0x00;
  uint32 address = 0x00;
...
void USART_DMACmd(USART_TypeDef* USARTx, uint16 USART_DMAReq, FunctionalState NewState)
{
  ...
  else
  {
    ...
    DMAR bits in the USART CR3 register */
    USARTx->CR3 &= (uint16)~USART_DMAReq;
  }
}
...
{
  /* Receive Data */
  return (uint16)(USARTx->DR & (uint16)0x01FF);
}
...
{    
  ...
  USARTx->GTPR |= (uint16)((uint16)USART_GuardTime << 0x08);
}
...
FlagStatus USART_GetFlagStatus(USART_TypeDef* USARTx, uint16 USART_FLAG)
{
  ...
  if ((USARTx->SR & USART_FLAG) != (uint16)RESET)
  {
    bitstatus = SET;
  }
  else
...
{
  uint32 bitpos = 0x00, itmask = 0x00, usartreg = 0;
  ...
  bitpos = (uint32)0x01 << bitpos;
  bitpos &= USARTx->SR;

  if ((itmask != (uint16)RESET)&&(bitpos != (uint16)RESET))
  {
    bitstatus = SET;
  }
  else
...
{
  uint32 bitpos = 0x00, itmask = 0x00;
