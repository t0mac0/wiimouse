...
/* Private define ------------------------------------------------------------*/
/* extern variables-----------------------------------------------------------*/
extern uint32 SCSI_LBA;
extern uint32 SCSI_BlkLen;
/* Private variables ---------------------------------------------------------*/
uint16 LUT[1024]; //Look Up Table Buffer
WRITE_STATE Write_State;
BLOCK_STATE Block_State;
NAND_ADDRESS wAddress, fAddress;
uint16  phBlock, LogAddress, Initial_Page, CurrentZone = 0;
uint16  Written_Pages = 0;
...
static uint16 NAND_Write_Cleanup(void);
SPARE_AREA ReadSpareArea(uint32 address);
static uint16 NAND_Copy(NAND_ADDRESS Address_Src, NAND_ADDRESS Address_Dest, uint16 PageToCopy);
static NAND_ADDRESS NAND_ConvertPhyAddress(uint32 Address);
static uint16 NAND_BuildLUT(uint8 ZoneNbr);
...
uint16 NAND_Write(uint32 Memory_Offset, uint32 *Writebuff, uint16 Transfer_Length)
{
  ...
  {/* Idle state */
    ...
    { /* USED BLOCK */
      ...
      /* write the new page */
      FSMC_NAND_WriteSmallPage((uint8 *)Writebuff, fAddress, PAGE_TO_WRITE);
      Written_Pages++;
    ...
    {/* UNUSED BLOCK */
      ...
      /* write the new page */
      wAddress.Block = phBlock & 0x3FF;
      FSMC_NAND_WriteSmallPage( (uint8 *)Writebuff , wAddress, PAGE_TO_WRITE);
...
uint16 NAND_Read(uint32 Memory_Offset, uint32 *Readbuff, uint16 Transfer_Length)
{
  ...
  else
  {
    ...
    FSMC_NAND_ReadSmallPage ( (uint8 *)Readbuff , phAddress, Transfer_Length / 512);
  }
  return NAND_OK;
}
...
static uint16 NAND_Copy (NAND_ADDRESS Address_Src, NAND_ADDRESS Address_Dest, uint16 PageToCopy)
{
  ...
  {
    FSMC_NAND_ReadSmallPage  ((uint8 *)Copybuff, Address_Src , 1 );
    FSMC_NAND_WriteSmallPage ((uint8 *)Copybuff, Address_Dest, 1);
...
static uint16 NAND_Write_Cleanup (void)
{
  ...
  if ( Block_State == OLD_BLOCK )
  {
    ...
    fAddress.Page     = 0x00;
    FSMC_NAND_WriteSpareArea( (uint8 *)tempSpareArea , fAddress , 1);
  ...
  {/* unused block case */
    ...
    wAddress.Page     = 0x00;
    FSMC_NAND_WriteSpareArea((uint8 *)tempSpareArea , wAddress, 1);
    NAND_CleanLUT(wAddress.Zone);
  }
  return NAND_OK;
}
  ...
  *****************************************************************************/
  ...
  while (pCurrentBlock < MAX_PHY_BLOCKS_PER_ZONE)
  {
    ...
    if ((SpareArea.DataStatus == 0) || (SpareArea.BlockStatus == 0))
    {
      ...
      LUT[pBadBlock--]    |= pCurrentBlock | (uint16)BAD_BLOCK ;
      LUT[pCurrentBlock] &= (uint16)~FREE_BLOCK;
