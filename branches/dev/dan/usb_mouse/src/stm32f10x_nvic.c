...
void NVIC_Init(NVIC_InitTypeDef* NVIC_InitStruct)
{
  ...
  if (NVIC_InitStruct->NVIC_IRQChannelCmd != DISABLE)
  {
    ...
    tmppriority = ((uint32)tmppriority) << ((NVIC_InitStruct->NVIC_IRQChannel & (uint32)0x03) * 0x08);
    
    tmpreg = NVIC->Priority[(NVIC_InitStruct->NVIC_IRQChannel >> 0x02)];
    tmpmask = (uint32)0xFF << ((NVIC_InitStruct->NVIC_IRQChannel & (uint32)0x03) * 0x08);
    tmpreg &= ~tmpmask;
    tmppriority &= tmpmask;  
    tmpreg |= tmppriority;
    ...
    NVIC->Enable[(NVIC_InitStruct->NVIC_IRQChannel >> 0x05)] =
      (uint32)0x01 << (NVIC_InitStruct->NVIC_IRQChannel & (uint32)0x1F);
  }
  else
  {
