...
static void NunchuckTask( void *pvParameters );
...
int main( void )
{
//	TIM1_TimeBaseInitTypeDef timeBase;
...
//
//	timeBase.TIM1_ClockDivision = 0;
//	timeBase.TIM1_CounterMode = TIM1_CounterMode_Up;
//	timeBase.TIM1_Period = 36000;
//	timeBase.TIM1_Prescaler = 2;
//	timeBase.TIM1_RepetitionCounter = 0;
//
//	//TIM1_InternalClockConfig();
//	//TIM1_UpdateRequestConfig(TIM1_UpdateSource_Regular);
//	TIM1_TimeBaseInit(&timeBase);
...
//	TIM1_Cmd(ENABLE);
	...
	xTaskCreate( prvCheckTask, ( signed char * ) "Check", configMINIMAL_STACK_SIZE, NULL, mainCHECK_TASK_PRIORITY, NULL );
		...
		xTaskCreate( NunchuckTask, ( signed char * ) "Nunchuck", configMINIMAL_STACK_SIZE, NULL, 5, NULL );
	//
...
static void prvCheckTask( void *pvParameters )
{
	...
	for( ;; )
	{
		...
		{
			printf("xAreQueuePeekTasksStillRunning: FALSE\n");
			ulTicksToWait = mainCHECK_DELAY_ERROR;
		}
		else if( xAreBlockingQueuesStillRunning() != pdTRUE )
		{
			printf("xAreBlockingQueuesStillRunning: FALSE\n");
			ulTicksToWait = mainCHECK_DELAY_ERROR;
		}
		else if( xAreSemaphoreTasksStillRunning() != pdTRUE )
		{
			printf("xAreSemaphoreTasksStillRunning: FALSE\n");
			ulTicksToWait = mainCHECK_DELAY_ERROR;
		}
		else if( xAreIntegerMathsTaskStillRunning() != pdTRUE )
		{
			printf("xAreIntegerMathsTaskStillRunning: FALSE\n");
			ulTicksToWait = mainCHECK_DELAY_ERROR;
		}
		else if( xAreRecursiveMutexTasksStillRunning() != pdTRUE )
		{
			//printf("xAreRecursiveMutexTasksStillRunning: FALSE\n");
			ulTicksToWait = mainCHECK_DELAY_ERROR;
		}
...
static void NunchuckTask( void *pvParameters )
{
	...
	while(1)
	{
		...
		if(nunchuck_read())
		{
			nunchuck_get_joystick((u8*)&x, (u8*)&y);
...
what the optimisation level. */
	...
	for( ;; )
	{
		...
		printf("%s %d\n","This is a test task string: tick:", tick);
		vTaskDelay(1000);
	}
}
/*-----------------------------------------------------------*/
