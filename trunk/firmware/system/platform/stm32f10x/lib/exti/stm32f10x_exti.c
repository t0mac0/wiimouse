...
/* Private typedef -----------------------------------------------------------*/
/* Private define ------------------------------------------------------------*/
#define EXTI_LineNone    ((uint32)0x00000)  /* No interrupt selected */
...
void EXTI_Init(EXTI_InitTypeDef* EXTI_InitStruct)
{
  ...
  {
    *(uint32 *)(EXTI_BASE + (uint32)EXTI_InitStruct->EXTI_Mode)|= EXTI_InitStruct->EXTI_Line;
    ...
    {
      *(uint32 *)(EXTI_BASE + (uint32)EXTI_InitStruct->EXTI_Trigger)|= EXTI_InitStruct->EXTI_Line;
    }
  }
  else
  {
...
FlagStatus EXTI_GetFlagStatus(uint32 EXTI_Line)
{
  ...
  if ((EXTI->PR & EXTI_Line) != (uint32)RESET)
  {
    bitstatus = SET;
  }
  else
...
ITStatus EXTI_GetITStatus(uint32 EXTI_Line)
{
  ...
  if (((EXTI->PR & EXTI_Line) != (uint32)RESET) && enablestatus)
  {
    bitstatus = SET;
  }
  else
