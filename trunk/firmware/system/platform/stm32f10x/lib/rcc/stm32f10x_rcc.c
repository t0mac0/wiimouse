...
#define CR_HSEBYP_Reset           ((uint32)0xFFFBFFFF)
#define CR_HSEBYP_Set             ((uint32)0x00040000)
#define CR_HSEON_Reset            ((uint32)0xFFFEFFFF)
#define CR_HSEON_Set              ((uint32)0x00010000)
#define CR_HSITRIM_Mask           ((uint32)0xFFFFFF07)

/* CFGR register bit mask */
#define CFGR_PLL_Mask             ((uint32)0xFFC0FFFF)
...
#define CFGR_SW_Mask              ((uint32)0xFFFFFFFC)
#define CFGR_HPRE_Reset_Mask      ((uint32)0xFFFFFF0F)
#define CFGR_HPRE_Set_Mask        ((uint32)0x000000F0)
#define CFGR_PPRE1_Reset_Mask     ((uint32)0xFFFFF8FF)
#define CFGR_PPRE1_Set_Mask       ((uint32)0x00000700)
#define CFGR_PPRE2_Reset_Mask     ((uint32)0xFFFFC7FF)
#define CFGR_PPRE2_Set_Mask       ((uint32)0x00003800)
#define CFGR_ADCPRE_Reset_Mask    ((uint32)0xFFFF3FFF)
#define CFGR_ADCPRE_Set_Mask      ((uint32)0x0000C000)

/* CSR register bit mask */
#define CSR_RVMF_Set              ((uint32)0x01000000)

/* RCC Flag Mask */
#define FLAG_Mask                 ((uint8)0x1F)

/* Typical Value of the HSI in Hz */
#define HSI_Value                 ((uint32)8000000)
...
/* Private macro -------------------------------------------------------------*/
/* Private variables ---------------------------------------------------------*/
static ucint8 APBAHBPrescTable[16] = {0, 0, 0, 0, 1, 2, 3, 4, 1, 2, 3, 4, 6, 7, 8, 9};
static ucint8 ADCPrescTable[4] = {2, 4, 6, 8};
...
void RCC_AdjustHSICalibrationValue(uint8 HSICalibrationValue)
{
	...
	tmpreg |= (uint32)HSICalibrationValue << 3;
...
void RCC_HSICmd(FunctionalState NewState)
{
	...
	*(vuint32 *) CR_HSION_BB = (uint32)NewState;
}
...
void RCC_PLLCmd(FunctionalState NewState)
{
	...
	*(vuint32 *) CR_PLLON_BB = (uint32)NewState;
}
...
{
	return ((uint8)(RCC->CFGR & CFGR_SWS_Mask));
}
...
void RCC_ITConfig(uint8 RCC_IT, FunctionalState NewState)
{
	...
	if (NewState != DISABLE)
	{
		...
		*(vuint8 *) 0x40021009 |= RCC_IT;
	}
	else
	{
		/* Perform Byte access to RCC_CIR[12:8] bits to disable the selected interrupts */
		*(vuint8 *) 0x40021009 &= ~(uint32)RCC_IT;
	}
}
...
void RCC_USBCLKConfig(uint32 RCC_USBCLKSource)
{
	...
	*(vuint32 *) CFGR_USBPRE_BB = RCC_USBCLKSource;
}
...
void RCC_LSEConfig(uint32 RCC_LSE)
{
	...
	*(vuint8 *) BDCR_BASE = RCC_LSE_OFF;
	...
	case RCC_LSE_ON:
		/* Set LSEON bit */
		*(vuint8 *) BDCR_BASE = RCC_LSE_ON;
		break;

	case RCC_LSE_Bypass:
		/* Set LSEBYP and LSEON bits */
		*(vuint8 *) BDCR_BASE = RCC_LSE_Bypass | RCC_LSE_ON;
		break;
...
void RCC_LSICmd(FunctionalState NewState)
{
	...
	*(vuint32 *) CSR_LSION_BB = (uint32)NewState;
}
...
void RCC_RTCCLKCmd(FunctionalState NewState)
{
	...
	*(vuint32 *) BDCR_RTCEN_BB = (uint32)NewState;
}
...
void RCC_GetClocksFreq(RCC_ClocksTypeDef* RCC_Clocks)
{
	...
	case 0x04:  /* HSE used as system clock */
		RCC_Clocks->SYSCLK_Frequency = HSE_VALUE; // TODO this should be set externally
		break;
...
void RCC_BackupResetCmd(FunctionalState NewState)
{
	...
	*(vuint32 *) BDCR_BDRST_BB = (uint32)NewState;
}
...
void RCC_ClockSecuritySystemCmd(FunctionalState NewState)
{
	...
	*(vuint32 *) CR_CSSON_BB = (uint32)NewState;
}
...
FlagStatus RCC_GetFlagStatus(uint8 RCC_FLAG)
{
	...
	if ((statusreg & ((uint32)1 << tmp)) != (uint32)RESET)
	{
		bitstatus = SET;
	}
	else
...
ITStatus RCC_GetITStatus(uint8 RCC_IT)
{
	...
	if ((RCC->CIR & RCC_IT) != (uint32)RESET)
	{
		bitstatus = SET;
	}
	else
