...
uint8 *Standard_GetInterface(uint16 Length)
{
    ...
    return (uint8 *)&pInformation->Current_AlternateSetting;
}
...
uint8 *Standard_GetStatus(uint16 Length)
{
    ...
    {
        /*Get Device Status */
        uint8 Feature = pInformation->Current_Feature;
    ...
    else if (Type_Recipient == (STANDARD_REQUEST | ENDPOINT_RECIPIENT))
    {
        ...
        uint8 wIndex0 = pInformation->USBwIndex0;
...
{
#ifdef STM32F10X_CL
    PCD_EP_SetAddress ((uint8)Val);
#else
    uint32 i;
    uint32 nEP = Device_Table.Total_Endpoint;
...
#else
        HwUsbPMAToUserBufferCopy(Buffer, GetEPRxAddr(ENDP0), Length);
#endif  /* STM32F10X_CL */
    }
...
#else
    HwUsbUserToPMABufferCopy(DataBuffer, GetEPTxAddr(ENDP0), Length);
#endif /* STM32F10X_CL */
...
void NoData_Setup0(void)
{
    ...
    uint32 RequestNo = pInformation->USBbRequest;
    uint32 ControlState;
...
void Data_Setup0(void)
{
    ...
    {
        /* Device ==> Host */
        vuint32 wLength = pInformation->USBwLength;
