...
------------------------------------------------------------------------------*/
uint8 Bot_State;
uint8 Bulk_Data_Buff[MASS_STORAGE_MAX_PACKET_SIZE];  /* data buffer*/
uint16 Data_Len;
Bulk_Only_CBW CBW;
Bulk_Only_CSW CSW;
...
void Mass_Storage_Out (void)
{
  ...
  Data_Len = HW_USB_Read(EP2_OUT, Bulk_Data_Buff, FALSE);
...
{
  HW_USB_Write(EP1_IN, Data_Pointer, Data_Len, TRUE);
...
void Set_CSW (uint8 CSW_Status, uint8 Send_Permission)
{
  ...
  HW_USB_Write(EP1_IN, ((uint8 *)& CSW), CSW_DATA_LENGTH, FALSE);
