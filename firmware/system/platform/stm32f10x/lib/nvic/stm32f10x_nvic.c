...
/* Private typedef -----------------------------------------------------------*/
/* Private define ------------------------------------------------------------*/
#define AIRC_VECTKEY_MASK    ((uint32)0x05FA0000)
...
void NVIC_Init(NVIC_InitTypeDef* NVIC_InitStruct)
{
  ...
  if (NVIC_InitStruct->NVIC_IRQChannelCmd != DISABLE)
  {
    ...
    tmppriority = (0x700 - (SCB->AIRC & (uint32)0x700))>> 0x08;
...
{
  return ((uint16)((SCB->IRQControlState & (uint32)0x003FF000) >> 0x0C));
}
...
void NVIC_SetIRQChannelPendingBit(uint8 NVIC_IRQChannel)
{
  ...
  *(uint32*)0xE000EF00 = (uint32)NVIC_IRQChannel;
}
...
void NVIC_ClearIRQChannelPendingBit(uint8 NVIC_IRQChannel)
{
  ...
  NVIC->Clear[(NVIC_IRQChannel >> 0x05)] = (uint32)0x01 << (NVIC_IRQChannel & (uint32)0x1F);
}
...
{
  return ((uint16)(SCB->IRQControlState & (uint32)0x3FF));
}
...
*******************************************************************************/
ITStatus NVIC_GetIRQChannelActiveBitStatus(uint8 NVIC_IRQChannel)
{
...
{
  SCB->AIRC = AIRC_VECTKEY_MASK | (uint32)0x01;
}
...
* Return         : None
*******************************************************************************/
void NVIC_SystemLPConfig(uint8 LowPowerMode, FunctionalState NewState)
{
  /* Check the parameters */
  ...
  {
    SCB->SysCtrl &= (uint32)(~(uint32)LowPowerMode);
  }
}
...
void NVIC_SystemHandlerConfig(uint32 SystemHandler, FunctionalState NewState)
{
  ...
  tmpreg =  (uint32)0x01 << (SystemHandler & (uint32)0x1F);
...
* Return         : None
*******************************************************************************/
void NVIC_SystemHandlerPriorityConfig(uint32 SystemHandler, uint8 SystemHandlerPreemptionPriority,
                                      uint8 SystemHandlerSubPriority)
{
  uint32 tmp1 = 0x00, tmp2 = 0xFF, handlermask = 0x00;
  uint32 tmppriority = 0x00;
  ...
  tmppriority = (uint32)SystemHandlerPreemptionPriority << tmp1;
  tmppriority |=  SystemHandlerSubPriority & tmp2;

  tmppriority = tmppriority << 0x04;
  tmp1 = SystemHandler & (uint32)0xC0;
  tmp1 = tmp1 >> 0x06; 
  tmp2 = (SystemHandler >> 0x08) & (uint32)0x03;
  tmppriority = tmppriority << (tmp2 * 0x08);
  handlermask = (uint32)0xFF << (tmp2 * 0x08);
...
void NVIC_SetSystemHandlerPendingBit(uint32 SystemHandler)
{
  ...
  tmp = SystemHandler & (uint32)0x1F;
  /* Set the corresponding System Handler pending bit */
  SCB->IRQControlState |= ((uint32)0x01 << tmp);
}
...
void NVIC_ClearSystemHandlerPendingBit(uint32 SystemHandler)
{
  ...
  SCB->IRQControlState |= ((uint32)0x01 << (tmp - 0x01));
}
...
ITStatus NVIC_GetSystemHandlerActiveBitStatus(uint32 SystemHandler)
{
  ...
  tmppos = (SystemHandler >> 0x0E) & (uint32)0x0F;
...
{
  uint32 faultsources = 0x00;
  uint32 tmpreg = 0x00, tmppos = 0x00;

  /* Check the parameters */
  ASSERT(IS_FAULT_SOURCE_SYSTEM_HANDLER(SystemHandler));
  
  tmpreg = (SystemHandler >> 0x12) & (uint32)0x03;
  tmppos = (SystemHandler >> 0x14) & (uint32)0x03;
  ...
  else if (tmpreg == 0x01)
  {
    ...
    if (tmppos != 0x02)
    {
      faultsources &= (uint32)0x0F;
    }
    else
    {
      faultsources &= (uint32)0xFF;
    }
  }
  else
  {
...
{
  uint32 faultaddress = 0x00;
  uint32 tmp = 0x00;

  /* Check the parameters */
  ASSERT(IS_FAULT_ADDRESS_SYSTEM_HANDLER(SystemHandler));
  
  tmp = (SystemHandler >> 0x16) & (uint32)0x01;
