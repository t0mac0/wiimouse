...
/* SPI CRCNext mask */
#define CR1_CRCNext_Set      ((uint16)0x1000)

/* SPI CRCEN mask */
#define CR1_CRCEN_Set        ((uint16)0x2000)
#define CR1_CRCEN_Reset      ((uint16)0xDFFF)
...
FlagStatus SPI_GetFlagStatus(SPI_TypeDef* SPIx, uint16 SPI_FLAG)
{
  ...
  if ((SPIx->SR & SPI_FLAG) != (uint16)RESET)
  {
    /* SPI_FLAG is set */
    bitstatus = SET;
  }
...
ITStatus SPI_GetITStatus(SPI_TypeDef* SPIx, uint8 SPI_IT)
{
  ...
  uint16 itpos = 0, itmask = 0, enablestatus = 0;

  /* Check the parameters */
  ASSERT(IS_SPI_GET_IT(SPI_IT));

  /* Get the SPI IT index */
  itpos = (uint16)((uint16)0x01 << (SPI_IT & (uint8)0x0F));

  /* Get the SPI IT index */
  itmask = SPI_IT >> 4;
  /* Set the IT mask */
  itmask = (uint16)((uint16)0x01 << itmask);
  /* Get the SPI_IT enable bit status */
  enablestatus = (SPIx->CR2 & itmask) ;

  /* Check the status of the specified SPI interrupt */
  if (((SPIx->SR & itpos) != (uint16)RESET) && enablestatus)
  {
    /* SPI_IT is set */
    bitstatus = SET;
  }
